module Prism
  class Calculator::Corrugated::RubberCostFormula
    ...
    def component
      @component ||= Prism::Component.find_by(code: COMPONENT_CODE)
    end

    def rubber_ratecard
      Prism::PartnerRatecard.joins(:component).find_by(partner_id: @partner.id, printing_type: 'corrugated', 'components.id': component&.id)
    end
