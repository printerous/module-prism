module Prism
  class Calculator::Corrugated::PrintCostFormula < Calculator::Corrugated::PostPressFormula
    ...
    def component_a
      @component_a ||= Prism::Component.find_by(code: "PRINT_COST_CORRUGATED_#{@product.colors.first}")
    end

    def ratecard_a
      @ratecard_a ||= Prism::PartnerRatecard.joins(:component).find_by(partner_id: @partner.id, version: 'ALL', 'components.id': component_a&.id)
    end
    ...
    def component_b
      @component_b ||= Prism::Component.find_by(code: "PRINT_COST_CORRUGATED_#{@product.colors.last}")
    end

    def ratecard_b
      @ratecard_b ||= Prism::PartnerRatecard.joins(:component).find_by(partner_id: @partner.id, version: 'ALL', 'components.id': component_b&.id)
    end
