...
module Prism
  class Calculator::Offset::Time::PrintingTimeFormula < Prism::Calculator::Offset::Time::TimeFormula
    NAME = 'PrintingTime'
    COMPONENTS = %w[TIME_PLATE_PREPARATION TIME_MACHINE_PREPARATION SPEED_PRINTING].freeze

    # get total material quantity
    attr_reader :total_material_quantity

    # get time components for machine
    def components
      @components ||= Prism::Component.where(code: COMPONENTS)
    end
    ...
    def machine_times
      @machine_times ||= begin
        ...
        components.each do |component|
          ratecard = Prism::PartnerRatecard.joins(:component).find_by(partner_id: @partner.id, version: @machine.code, 'components.id': component.id)
    ...
    def material_ratecard
      @material_ratecard ||= Prism::PartnerRatecard.joins(:component).find_by(partner_id: @partner.id, version: @paper.plano.code, 'components.id': @product.material.id)
    end
    ...
    def machine_preparation_time
      machine_preparation_components = %w[TIME_PLATE_PREPARATION TIME_MACHINE_PREPARATION]
      @machine_preparation_time ||= begin
        component_times = machine_times.select { |key, _value| machine_preparation_components.include?(key) }
